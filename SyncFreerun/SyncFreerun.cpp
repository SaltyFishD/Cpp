...
void Sample::findMaster()
{
    ...
    } while (nMaster > 1);    // Repeat until there is at maximum 1 master left !
...
void Sample::syncCameras() {
    ...
    for (size_t camIdx = 0; camIdx < m_NumCams; camIdx++)
    {
        // Check all slaves for deviation from master clock.
...
void Sample::setTriggerDelays() {
    ...
    for (size_t camIdx = 0; camIdx < m_Cameras.size(); camIdx++)
    {
        ...
        if (camIdx == 0)
        {
            ...
            // If calculated value is greater than maximum supported rate, 
            // adjust it. 
